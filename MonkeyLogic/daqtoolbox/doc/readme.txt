  ...
  interface on 64-bit MATLAB nor in the future releases of MATLAB from R2016a.
  Therefore, NIMH DAQ is developed to extend the legacy interface to 64-bit
  MATLAB and continue to support it so that users can keep running mission-
  critical MATLAB code written based on the legacy DAQ interface.
  ...
  DAQ also allows you to load multiple waveforms for the same analogoutput
  channel and then choose which one to send out.  (See the example scripts.)
  ...
  - NI-DAQmx Software (https://www.ni.com/dataacquisition/nidaqmx.htm)
  
  - Sound cards (to use 'winsound')
  
  - parallel ports (either on-board or PCI, if you want to use 'parallel')
